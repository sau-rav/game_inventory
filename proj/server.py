...
def index():
	...
	if query == '':
		game_data = db.get_game_data('')
	else:
		game_data = db.get_game_data(query)
	...
	if query == '':
		...
		return render_template('index.html', game_list = game_list, disp_list = disp_list, page = page, filter = query)
